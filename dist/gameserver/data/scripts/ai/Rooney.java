...
public class Rooney extends DefaultAI
{
	...
	protected void onEvtThink()
	{
		if(!_discoveredByPlayer)
		{
			...
			final List<Player> players = World.getAroundPlayers(actor, 1500, 400);
			if(!players.isEmpty())
			{
				_discoveredByPlayer = true;
				Location loc;
				for(loc = Rooney.LOCATIONS[Rnd.get(Rooney.LOCATIONS.length)]; actor.getX() == loc.x; loc = Rooney.LOCATIONS[Rnd.get(Rooney.LOCATIONS.length)])
				{}
				ThreadPoolManager.getInstance().schedule(new ShoutTask(), 100L);
				ThreadPoolManager.getInstance().schedule(new TeleportTask(loc), 600000L);
			}
		}
		super.onEvtThink();
	}
	...
	static
	{
		...
		SHOUTS = new String[] {
				"Welcome!",
				"Hurry hurry",
				"I am not that type of person who stays in one place for a long time",
				"It's hard for me to keep standing like this",
				"Why don't I go that way this time" };
	}

	private final class ShoutTask implements Runnable
	{
		private int _idx;
		...
		public final void run()
		{
			...
			{
				Functions.npcShout(getActor(), Rooney.SHOUTS[_idx], 0);
				++_idx;
