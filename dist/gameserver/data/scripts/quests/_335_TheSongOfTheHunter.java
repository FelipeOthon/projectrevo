...
public class _335_TheSongOfTheHunter extends Quest implements ScriptFile
{
	...
	public _335_TheSongOfTheHunter()
	{
		...
		this.addTalkId(new int[] { 30746 });
		this.addTalkId(new int[] { 30745 });
		this.addKillId(new int[] { 27140 });
		this.addKillId(new int[] { 27141 });
		this.addKillId(new int[] { 27142 });
		this.addKillId(new int[] { 27144 });
		this.addKillId(new int[] { 27145 });
		this.addKillId(new int[] { 27146 });
		this.addKillId(new int[] { 27147 });
		this.addKillId(new int[] { 27148 });
		this.addKillId(_335_TheSongOfTheHunter.q_blood_crystal_lizardmen);
		for(final int[][] ItemsCond : _335_TheSongOfTheHunter.Items_1st_Circle)
		{
			addQuestItem(ItemsCond[0]);
			for(int i = 2; i < ItemsCond.length; ++i)
				this.addKillId(new int[] { ItemsCond[i][0] });
		}
		...
		{
			addQuestItem(new int[] { r.request_id });
			addQuestItem(new int[] { r.request_item });
		...
		addQuestItem(new int[] { 3692 });
		addQuestItem(new int[] { 3693 });
		addQuestItem(new int[] { 3694 });
		addQuestItem(new int[] { 3695 });
		addQuestItem(new int[] { 3696 });
		addQuestItem(new int[] { 3697 });
		addQuestItem(new int[] { 3471 });
			...
			for(final int[][] ItemsCond : Items_Circle)
				for(int i = 2; i < ItemsCond.length; ++i)
					if(npcId == ItemsCond[i][0])
						st.rollAndGive(ItemsCond[0][0], 1, 1, ItemsCond[1][0], ItemsCond[i][1], true);
			if(st.getQuestItemsCount(3695) > 0L)
			{
				...
				else if(npcId == 27140)
				{
					if(Hakas_Head_count == 0L)
						st.rollAndGive(3711, 1, 1, 1, 100.0, false);
				}
				else if(npcId == 27141)
				{
					if(Jakas_Head_count == 0L)
						st.rollAndGive(3712, 1, 1, 1, 100.0, false);
				}
				else if(npcId == 27142)
				{
					if(Markas_Head_count == 0L)
						st.rollAndGive(3713, 1, 1, 1, 100.0, false);
				}
			...
			else if(st.getQuestItemsCount(3696) > 0L)
			{
				...
				else if(npcId == 27144)
				{
					if(Athus_Head_count == 0L)
						st.rollAndGive(3722, 1, 1, 1, 100.0, false);
				}
				else if(npcId == 27145)
				{
					if(Lankas_Head_count == 0L)
						st.rollAndGive(3723, 1, 1, 1, 100.0, false);
				}
				else if(npcId == 27146)
				{
					if(Triskas_Head_count == 0L)
						st.rollAndGive(3724, 1, 1, 1, 100.0, false);
				}
				else if(npcId == 27147)
				{
					if(Moturas_Head_count == 0L)
						st.rollAndGive(3725, 1, 1, 1, 100.0, false);
				}
				else if(npcId == 27148 && Kalaths_Head_count == 0L)
					st.rollAndGive(3726, 1, 1, 1, 100.0, false);
			}
		}
		if(st.getQuestItemsCount(3692) > 0L || st.getQuestItemsCount(3693) > 0L)
		{
					...
					for(final int lizardmen_id : _335_TheSongOfTheHunter.q_blood_crystal_lizardmen)
						if(npcId == lizardmen_id)
							if(Rnd.chance(50))
							{
								...
								st.playSound(Blood_Crystal_Level < 6 ? Quest.SOUND_MIDDLE : Quest.SOUND_JACKPOT);
			...
			if(request != null)
			{
				if(request.droplist.containsKey(npcId))
					st.rollAndGive(request.request_item, 1, 1, request.request_count, request.droplist.get(npcId), true);
	...
	static
	{
		q_blood_crystal = new int[] { 3708, 3698, 3699, 3700, 3701, 3702, 3703, 3704, 3705, 3706, 3707 };
		q_blood_crystal_lizardmen = new int[] { 20578, 20579, 20580, 20581, 20582, 20641, 20642, 20643, 20644, 20645 };
		Items_1st_Circle = new int[][][] {
				{ { 3709 }, { 40 }, { 20550, 75 } },
				{ { 3710 }, { 20 }, { 20581, 50 } },
				{ { 3711, 3712, 3713 }, { 3 } },
				{ { 3714 }, { 1 }, { 27143, 100 } },
	...
	public static class Request
	{
		...
		public Request(final int requestid, final int requestitem, final int requestcount, final int rewardadena, final String txt)
		{
			...
			spawnlist = new HashMap<Integer, int[]>();
