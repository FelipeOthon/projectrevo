...
public class OpenSealedBox
{
	...
	public OpenSealedBox(final QuestState st, final int count)
	{
		result = "";
		takecount = 0;
		rewards = new HashMap<Integer, Long>();
		this.st = st;
		if(count < 1)
			return;
		takecount = count;
		...
		for(int i = 0; i < not_disintegrated; ++i)
			OpenSealedBox.rewardgroups[Rnd.get(OpenSealedBox.rewardgroups.length)].apply(rewards);
		if(rewards.size() == 0)
		{
	...
	static
	{
		rewardgroups = new RewardGroup[] { new RewardAdena(), new RewardRes1(), new RewardRes2(), new RewardEnchants(), new RewardParts() };
		counts = new int[] { 1, 5, 10 };
	}
