...
public class _219_TestimonyOfFate extends Quest implements ScriptFile
{
	...
	public _219_TestimonyOfFate()
	{
		...
		for(int i = 0; i < _219_TestimonyOfFate.DROPLIST_COND.length; ++i)
			this.addKillId(new int[] { _219_TestimonyOfFate.DROPLIST_COND[i][2] });
	...
	public String onEvent(final String event, final QuestState st, final NpcInstance npc)
	{
		...
		{
			List<NpcInstance> isQuest = GameObjectsStorage.getNpcs(false, 30613);
			if(!isQuest.isEmpty())
	...
	public String onTalk(final NpcInstance npc, final QuestState st)
	{
		...
		if(npcId == 30476)
		{
			...
			{
				List<NpcInstance> AldersSpiritObject = GameObjectsStorage.getNpcs(false, 30613);
				if(AldersSpiritObject.isEmpty())
				{
		...
		else if(npcId == 30613)
		{
			...
			st.set("cond", "11");
			st.setState(2);
			final QuestTimer timer = st.getQuestTimer("AldersSpirit_Fail");
			if(timer != null)
				timer.cancel();
			List<NpcInstance> isQuest = GameObjectsStorage.getNpcs(false, 30613);
			if(!isQuest.isEmpty())
		...
		else if(npcId == 30358)
		{
			...
			else if(cond == 18)
			{
				...
				{
					st.addExpAndSp(682735L, 45562L, true);
					st.giveItems(57, 123854L, Config.RATE_QUESTS_OCCUPATION_CHANGE);
		...
		for(int i = 0; i < _219_TestimonyOfFate.DROPLIST_COND.length; ++i)
			if(cond == _219_TestimonyOfFate.DROPLIST_COND[i][0] && npcId == _219_TestimonyOfFate.DROPLIST_COND[i][2] && (_219_TestimonyOfFate.DROPLIST_COND[i][3] == 0 || st.getQuestItemsCount(_219_TestimonyOfFate.DROPLIST_COND[i][3]) > 0L))
				if(_219_TestimonyOfFate.DROPLIST_COND[i][5] == 0)
					st.rollAndGive(_219_TestimonyOfFate.DROPLIST_COND[i][4], _219_TestimonyOfFate.DROPLIST_COND[i][7], _219_TestimonyOfFate.DROPLIST_COND[i][6], _219_TestimonyOfFate.QuestProf);
				else if(st.rollAndGive(_219_TestimonyOfFate.DROPLIST_COND[i][4], _219_TestimonyOfFate.DROPLIST_COND[i][7], _219_TestimonyOfFate.DROPLIST_COND[i][7], _219_TestimonyOfFate.DROPLIST_COND[i][5], _219_TestimonyOfFate.DROPLIST_COND[i][6], _219_TestimonyOfFate.QuestProf) && _219_TestimonyOfFate.DROPLIST_COND[i][1] != cond && _219_TestimonyOfFate.DROPLIST_COND[i][1] != 0)
				{
					st.set("cond", String.valueOf(_219_TestimonyOfFate.DROPLIST_COND[i][1]));
					st.setState(2);
				}
	...
	static
	{
		DROPLIST_COND = new int[][] {
				{ 6, 0, 20158, 3177, 3178, 10, 100, 1 },
				{ 6, 0, 20233, 3177, 3179, 10, 100, 1 },
				{ 6, 0, 20202, 3177, 3180, 10, 100, 1 },
				{ 6, 0, 20192, 3177, 3181, 10, 100, 1 },
				{ 6, 0, 20193, 3177, 3181, 10, 100, 1 },
				{ 6, 0, 20230, 3177, 3182, 10, 100, 1 },
				{ 6, 0, 20157, 3177, 3182, 10, 100, 1 },
				{ 6, 0, 20232, 3177, 3182, 10, 100, 1 },
				{ 6, 0, 20234, 3177, 3182, 10, 100, 1 },
				{ 17, 0, 20554, 3193, 3194, 10, 100, 1 },
				{ 17, 0, 20600, 3193, 3195, 10, 100, 1 },
				{ 17, 0, 20270, 3193, 3196, 10, 100, 1 },
				{ 17, 0, 20582, 3193, 3197, 10, 100, 1 },
				{ 17, 0, 27079, 3199, 3200, 10, 100, 1 } };
		_219_TestimonyOfFate.QuestProf = true;
	}
}
