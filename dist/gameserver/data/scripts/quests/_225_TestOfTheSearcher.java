...
public class _225_TestOfTheSearcher extends Quest implements ScriptFile
{
	...
	public _225_TestOfTheSearcher()
	{
		...
		for(int i = 0; i < _225_TestOfTheSearcher.DROPLIST_COND.length; ++i)
			this.addKillId(new int[] { _225_TestOfTheSearcher.DROPLIST_COND[i][2] });
	...
	public String onEvent(final String event, final QuestState st, final NpcInstance npc)
	{
		...
		{
			List<NpcInstance> isQuest = GameObjectsStorage.getNpcs(false, 30628);
			if(isQuest.isEmpty())
			{
		...
		if(npcId == 30690)
		{
			...
			else if(cond == 19)
			{
				...
				if(!st.getPlayer().getVarBoolean("prof2.3"))
				{
					...
					st.giveItems(57, 80093L, Config.RATE_QUESTS_OCCUPATION_CHANGE);
		...
		for(int i = 0; i < _225_TestOfTheSearcher.DROPLIST_COND.length; ++i)
			if(cond == _225_TestOfTheSearcher.DROPLIST_COND[i][0] && npcId == _225_TestOfTheSearcher.DROPLIST_COND[i][2] && (_225_TestOfTheSearcher.DROPLIST_COND[i][3] == 0 || st.getQuestItemsCount(_225_TestOfTheSearcher.DROPLIST_COND[i][3]) > 0L))
				if(_225_TestOfTheSearcher.DROPLIST_COND[i][5] == 0)
					st.rollAndGive(_225_TestOfTheSearcher.DROPLIST_COND[i][4], _225_TestOfTheSearcher.DROPLIST_COND[i][7], _225_TestOfTheSearcher.DROPLIST_COND[i][6], _225_TestOfTheSearcher.QuestProf);
				else if(st.rollAndGive(_225_TestOfTheSearcher.DROPLIST_COND[i][4], _225_TestOfTheSearcher.DROPLIST_COND[i][7], _225_TestOfTheSearcher.DROPLIST_COND[i][7], _225_TestOfTheSearcher.DROPLIST_COND[i][5], _225_TestOfTheSearcher.DROPLIST_COND[i][6], _225_TestOfTheSearcher.QuestProf) && _225_TestOfTheSearcher.DROPLIST_COND[i][1] != cond && _225_TestOfTheSearcher.DROPLIST_COND[i][1] != 0)
				{
					st.set("cond", String.valueOf(_225_TestOfTheSearcher.DROPLIST_COND[i][1]));
					st.setState(2);
				}
	...
	static
	{
		DROPLIST_COND = new int[][] {
				{ 3, 4, 20781, 0, 2787, 10, 100, 1 },
				{ 3, 4, 27094, 0, 2787, 10, 100, 1 },
				{ 10, 11, 20555, 0, 2797, 10, 100, 1 } };
		_225_TestOfTheSearcher.QuestProf = true;
	}
}
