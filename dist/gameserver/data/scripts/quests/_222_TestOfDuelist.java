...
public class _222_TestOfDuelist extends Quest implements ScriptFile
{
	...
	public _222_TestOfDuelist()
	{
		...
		{
			this.addKillId(new int[] { _222_TestOfDuelist.DROPLIST_COND[i][2] });
			addQuestItem(new int[] { _222_TestOfDuelist.DROPLIST_COND[i][4] });
		}
		addQuestItem(new int[] { 2763, 2764, 2765, 2766, 2767, 2779, 2780, 2781, 2782, 2783, 2778 });
	}
	...
	public String onTalk(final NpcInstance npc, final QuestState st)
	{
		...
		if(npcId == 30623)
			...
			else if(cond == 5)
			{
				...
				{
					st.addExpAndSp(474444L, 30704L, true);
					st.giveItems(57, 80000L, Config.RATE_QUESTS_OCCUPATION_CHANGE);
		...
		for(int i = 0; i < _222_TestOfDuelist.DROPLIST_COND.length; ++i)
			if(cond == _222_TestOfDuelist.DROPLIST_COND[i][0] && npcId == _222_TestOfDuelist.DROPLIST_COND[i][2] && (_222_TestOfDuelist.DROPLIST_COND[i][3] == 0 || st.getQuestItemsCount(_222_TestOfDuelist.DROPLIST_COND[i][3]) > 0L))
				if(_222_TestOfDuelist.DROPLIST_COND[i][5] == 0)
					st.rollAndGive(_222_TestOfDuelist.DROPLIST_COND[i][4], _222_TestOfDuelist.DROPLIST_COND[i][7], _222_TestOfDuelist.DROPLIST_COND[i][6], _222_TestOfDuelist.QuestProf);
				else if(st.rollAndGive(_222_TestOfDuelist.DROPLIST_COND[i][4], _222_TestOfDuelist.DROPLIST_COND[i][7], _222_TestOfDuelist.DROPLIST_COND[i][7], _222_TestOfDuelist.DROPLIST_COND[i][5], _222_TestOfDuelist.DROPLIST_COND[i][6], _222_TestOfDuelist.QuestProf) && _222_TestOfDuelist.DROPLIST_COND[i][1] != cond && _222_TestOfDuelist.DROPLIST_COND[i][1] != 0)
				{
					st.set("cond", String.valueOf(_222_TestOfDuelist.DROPLIST_COND[i][1]));
					st.setState(2);
				}
	...
	static
	{
		...
		_222_TestOfDuelist.QuestProf = true;
	}
}
