...
public class _220_TestimonyOfGlory extends Quest implements ScriptFile
{
	...
	public _220_TestimonyOfGlory()
	{
		...
		this.addTalkId(new int[] { 30642 });
		this.addTalkId(new int[] { 30515 });
		this.addTalkId(new int[] { 30501 });
		this.addTalkId(new int[] { 30615 });
		this.addTalkId(new int[] { 30616 });
		this.addTalkId(new int[] { 30617 });
		this.addTalkId(new int[] { 30618 });
		this.addTalkId(new int[] { 30619 });
		this.addTalkId(new int[] { 30571 });
		this.addTalkId(new int[] { 30565 });
		for(int i = 0; i < _220_TestimonyOfGlory.DROPLIST_COND.length; ++i)
			this.addKillId(new int[] { _220_TestimonyOfGlory.DROPLIST_COND[i][2] });
		this.addKillId(new int[] { 27080 });
		this.addKillId(new int[] { 27081 });
		this.addKillId(new int[] { 20778 });
		this.addKillId(new int[] { 20779 });
		this.addKillId(new int[] { 27086 });
	...
	public String onEvent(final String event, final QuestState st, final NpcInstance npc)
	{
		...
		{
			int spawn = 0;
			List<NpcInstance> isQuest = GameObjectsStorage.getNpcs(false, 27080);
			if(!isQuest.isEmpty())
				spawn = 1;
			isQuest = GameObjectsStorage.getNpcs(false, 27081);
			if(!isQuest.isEmpty())
				spawn = 1;
			if(spawn == 1)
			{
		...
		{
			List<NpcInstance> isQuest2 = GameObjectsStorage.getNpcs(false, 27082);
			if(!isQuest2.isEmpty())
			{
				final QuestTimer timer2 = st.getQuestTimer("Wait2");
				if(timer2 == null)
		...
		{
			List<NpcInstance> isQuest2 = GameObjectsStorage.getNpcs(false, 27083);
			if(!isQuest2.isEmpty())
			{
				final QuestTimer timer2 = st.getQuestTimer("Wait3");
				if(timer2 == null)
		...
		{
			List<NpcInstance> isQuest2 = GameObjectsStorage.getNpcs(false, 27080);
			if(!isQuest2.isEmpty())
				isQuest2.get(0).deleteMe();
			isQuest2 = GameObjectsStorage.getNpcs(false, 27081);
			if(!isQuest2.isEmpty())
				isQuest2.get(0).deleteMe();
			QuestTimer timer2 = st.getQuestTimer("Wait1");
			if(timer2 != null)
				timer2.cancel();
			timer2 = st.getQuestTimer("PashikasSonOfVoltarQuestMonster");
			if(timer2 != null)
				timer2.cancel();
		}
		...
		{
			List<NpcInstance> isQuest2 = GameObjectsStorage.getNpcs(false, 27082);
			for(NpcInstance npc1 : isQuest2)
				npc1.deleteMe();
			QuestTimer timer2 = st.getQuestTimer("Wait2");
			if(timer2 != null)
				timer2.cancel();
			timer2 = st.getQuestTimer("EnkuOrcOverlordQuestMonster");
			if(timer2 != null)
				timer2.cancel();
		}
		else if(event.equalsIgnoreCase("Wait3") || event.equalsIgnoreCase("MakumBugbearThugQuestMonster"))
		{
			...
			for(NpcInstance npc1 : isQuest2)
				npc1.deleteMe();
			QuestTimer timer2 = st.getQuestTimer("Wait3");
			if(timer2 != null)
				timer2.cancel();
			timer2 = st.getQuestTimer("MakumBugbearThugQuestMonster");
			if(timer2 != null)
				timer2.cancel();
		}
		else if(event.equalsIgnoreCase("Wait4") || event.equalsIgnoreCase("RevenantOfTantosChief"))
		{
			List<NpcInstance> isQuest2 = GameObjectsStorage.getNpcs(false, 27086);
			if(!isQuest2.isEmpty())
				isQuest2.get(0).deleteMe();
			QuestTimer timer2 = st.getQuestTimer("Wait4");
			if(timer2 != null)
				timer2.cancel();
			timer2 = st.getQuestTimer("RevenantOfTantosChief");
			if(timer2 != null)
				timer2.cancel();
		}
		return htmltext;
	...
	public String onTalk(final NpcInstance npc, final QuestState st)
	{
		...
		if(npcId == 30565 && cond == 11)
		{
			...
			{
				st.addExpAndSp(724113L, 48324L, true);
				st.giveItems(57, 131360L, Config.RATE_QUESTS_OCCUPATION_CHANGE);
	...
	public String onKill(final NpcInstance npc, final QuestState st)
	{
		...
		for(int i = 0; i < _220_TestimonyOfGlory.DROPLIST_COND.length; ++i)
			if(cond == _220_TestimonyOfGlory.DROPLIST_COND[i][0] && npcId == _220_TestimonyOfGlory.DROPLIST_COND[i][2] && (_220_TestimonyOfGlory.DROPLIST_COND[i][3] == 0 || st.getQuestItemsCount(_220_TestimonyOfGlory.DROPLIST_COND[i][3]) > 0L))
				if(_220_TestimonyOfGlory.DROPLIST_COND[i][5] == 0)
					st.rollAndGive(_220_TestimonyOfGlory.DROPLIST_COND[i][4], _220_TestimonyOfGlory.DROPLIST_COND[i][7], _220_TestimonyOfGlory.DROPLIST_COND[i][6], _220_TestimonyOfGlory.QuestProf);
				else if(st.rollAndGive(_220_TestimonyOfGlory.DROPLIST_COND[i][4], _220_TestimonyOfGlory.DROPLIST_COND[i][7], _220_TestimonyOfGlory.DROPLIST_COND[i][7], _220_TestimonyOfGlory.DROPLIST_COND[i][5], _220_TestimonyOfGlory.DROPLIST_COND[i][6], _220_TestimonyOfGlory.QuestProf) && _220_TestimonyOfGlory.DROPLIST_COND[i][1] != cond && _220_TestimonyOfGlory.DROPLIST_COND[i][1] != 0)
				{
					st.set("cond", String.valueOf(_220_TestimonyOfGlory.DROPLIST_COND[i][1]));
					st.setState(2);
				}
		...
		else if(cond == 4)
		{
			if(npcId == 27080)
			{
				final QuestTimer timer = st.getQuestTimer("PashikasSonOfVoltarQuestMonster");
				if(timer != null)
					timer.cancel();
				List<NpcInstance> isQuest = GameObjectsStorage.getNpcs(false, 27080);
				if(!isQuest.isEmpty())
			...
			{
				final QuestTimer timer = st.getQuestTimer("VultusSonOfVoltarQuestMonster");
				if(timer != null)
					timer.cancel();
				List<NpcInstance> isQuest = GameObjectsStorage.getNpcs(false, 27081);
				if(!isQuest.isEmpty())
					isQuest.get(0).deleteMe();
		...
		else if(cond == 6 && st.getQuestItemsCount(3219) >= 20L && st.getQuestItemsCount(3218) >= 20L)
		{
			...
			{
				List<NpcInstance> isQuest = GameObjectsStorage.getNpcs(false, 27086);
				if(isQuest.isEmpty())
				{
				...
				{
					final QuestTimer timer2 = st.getQuestTimer("Wait4");
					if(timer2 == null)
	...
	static
	{
		DROPLIST_COND = new int[][] {
				{ 1, 0, 20563, 3204, 3205, 10, 70, 1 },
				{ 1, 0, 20192, 3204, 3206, 10, 70, 1 },
				{ 1, 0, 20193, 3204, 3206, 10, 70, 1 },
				{ 1, 0, 20550, 3204, 3207, 10, 70, 1 },
				{ 4, 0, 20234, 3233, 3234, 30, 70, 1 },
				{ 4, 0, 27082, 3225, 3224, 4, 100, 1 },
				{ 4, 0, 27083, 3227, 3226, 2, 100, 1 },
				{ 6, 0, 20583, 3217, 3219, 20, 50, 1 },
				{ 6, 0, 20584, 3217, 3219, 20, 60, 1 },
				{ 6, 0, 20585, 3217, 3219, 20, 70, 1 },
				{ 6, 0, 20586, 3217, 3219, 20, 80, 1 },
				{ 6, 0, 20587, 3217, 3219, 20, 90, 1 },
				{ 6, 0, 20588, 3217, 3219, 20, 100, 1 },
				{ 6, 0, 20601, 3217, 3218, 20, 50, 1 },
				{ 6, 0, 20602, 3217, 3218, 20, 60, 1 } };
		_220_TestimonyOfGlory.QuestProf = true;
	}
}
