...
public final class ServerList extends L2LoginServerPacket
{
	...
	private static class ServerData
	{
		...
		ServerData(int serverId, InetAddress adress, int port, boolean pvp, boolean brackets, int type, int online, int maxPlayers, boolean status, int ageLimit)
		{
	...
	public ServerList(Account account)
	{
		...
		for(GameServer gs : GameServerManager.getInstance().getGameServers())
		{
			for(HostInfo host : gs.getHosts())
			{
				...
				_servers.add(new ServerData(host.getId(), adress, host.getPort(), gs.isPvp(), gs.isShowingBrackets(), gs.getServerType(), gs.getOnline(), gs.getMaxPlayers(), gs.isOnline(), gs.getAgeLimit()));
			}
		}
	}
