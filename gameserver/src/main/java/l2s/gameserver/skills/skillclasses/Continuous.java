		...
		for(final Creature target : targets)
			if(target != null)
			{
				...
				if(lethal1 > 0.0 && Rnd.chance(lethal1))
				{
					...
					{
						realTarget.reduceCurrentHp(realTarget.getCurrentCp(), activeChar, this, 0, false, true, true, false, true, false, false, true);
					...
					{
						realTarget.reduceCurrentHp(realTarget.getCurrentHp() / 2.0, activeChar, this, 0, false, true, true, false, true, false, false, true);
						activeChar.sendPacket(Msg.INSTANT_KILL);
					}
				}
					...
					if(realTarget.isPlayer())
					{
						realTarget.reduceCurrentHp(realTarget.getCurrentHp() + realTarget.getCurrentCp() - 1.0, activeChar, this, 0, false, true, true, false, true, false, false, true);
						realTarget.sendPacket(Msg.YOU_HAVE_BEEN_STRUCK_BY_THE_INSTANT_KILL_SKILL);
						activeChar.sendPacket(Msg.INSTANT_KILL);
					}
					else if(realTarget.isNpc() && !realTarget.isLethalImmune())
					{
						realTarget.reduceCurrentHp(realTarget.getCurrentHp() - 1.0, activeChar, this, 0, false, true, true, false, true, false, false, true);
						activeChar.sendPacket(Msg.INSTANT_KILL);
					}
